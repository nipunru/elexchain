  ...
  const renderWinner = (winner) => {
    return (
      <div
        ...
        style={{
          background: `linear-gradient(to bottom, rgba(255, 255, 255, 0.5), ${winner.colorCode}30)`,
        }}
      >
        <div className="winner-title">Winner!</div>
        <div className="winner-content">
          <div className="candidate-number">#{winner.candidateNumber}</div>
          ...
          <div className="candidate-details">
            ...
            <div className="party-slogan">
              <div
                className="party-box"
                style={{ background: winner.colorCode }}
              >
                {winner.electionParty}
              </div>
              <span className="winner-slogan">{winner.slogan}</span>
            </div>
          </div>
          <div className="vote-count">
            {winner.voteCount}
            <small style={{ fontSize: "20px" }}> Votes</small>
          </div>
        </div>
      </div>
    );
...
export function displayResults(candidates) {
  const renderResults = (candidate) => {
    return (
      <tr style={{ backgroundColor: `${candidate.colorCode}50` }}>
        <td>{candidate.candidateNumber}</td>
        <td>{candidate.header}</td>
        <td>{candidate.electionParty}</td>
